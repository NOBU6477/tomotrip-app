...
================================
■ シナリオL
お問い合わせフォーム（3系統入口・メール送信）
================================

目的：
ガイド/観光客/協賛店の3系統からお問い合わせフォームが
正しく動作し、メールが送信されることを確認する。

手順 L-1（タイプ別表示確認）：
1. /contact.html?type=guide&source=app を開く
2. 見出しが「ガイドの方のお問い合わせ」であることを確認
3. /contact.html?type=tourist&source=app を開く
4. 見出しが「観光客の方のお問い合わせ」であることを確認
5. /contact.html?type=sponsor&source=app を開く
6. 見出しが「協賛店の方のお問い合わせ」であることを確認

合格条件（PASS）：
- [ ] type=guide → 「ガイドの方のお問い合わせ」
- [ ] type=tourist → 「観光客の方のお問い合わせ」
- [ ] type=sponsor → 「協賛店の方のお問い合わせ」

手順 L-2（バリデーション確認）：
1. /contact.html?type=tourist&source=app を開く
2. 何も入力せずに「送信する」を押す
3. 名前/メール/メッセージにエラー表示が出ることを確認
4. 名前を入力する
5. 不正なメールアドレス（例: test@）を入力する
6. メッセージを5文字だけ入力する
7. 「送信する」を押す
8. メール形式エラー/メッセージ文字数エラーが表示されることを確認
...
1. /contact.html?type=tourist&source=app を開く
2. 名前：「テスト太郎」を入力
3. メール：「test@example.com」を入力
4. メッセージ：「これはテストメッセージです。10文字以上。」を入力
5. 「送信する」を押す
6. ローディング表示後、成功メッセージが表示されることを確認
7. （SENDGRID_API_KEY設定時）info@tomotrip.com にメールが届くことを確認
8. （SENDGRID_API_KEY未設定時）「テストモード」の警告メッセージが表示されることを確認

合格条件（PASS）：
- [ ] 正しい入力 → 201レスポンス
- [ ] 成功メッセージ表示
- [ ] 本番キー設定時 → info@tomotrip.com に届く

手順 L-4（送信後の状態維持確認）：
1. トップページで検索条件を設定し、2ページ目へ移動
2. ナビゲーションからお問い合わせページへ移動
3. 送信完了後、「← トップページに戻る」を押す
4. 検索条件・ページ番号が破壊されていないことを確認
...
- お問い合わせフォーム修正後はシナリオLを優先して確認すること。
